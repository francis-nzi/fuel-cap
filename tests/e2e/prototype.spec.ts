...
test("core prototype flow works without horizontal overflow", async ({ page }, testInfo) => {
  const bypassDevOverlay = testInfo.project.name === "customer-mobile";
  async function customerClick(locator: Locator) {
  ...
  await customerClick(page.getByRole("button", { name: "Lock price", exact: true }).last());
  ...
  await customerClick(page.getByRole("button", { name: "One brand" }));
  ...
  await customerClick(page.getByRole("button", { name: "Anywhere" }));
  await expect(page.getByText("£1.48", { exact: false }).first()).toBeVisible();
  await customerClick(page.getByRole("button", { name: "Confirm price lock" }));
  await expect(page.getByText("Active price locks")).toBeVisible();
  await expect(page.getByText("160 L", { exact: true })).toBeVisible();

  await customerClick(page.getByRole("button", { name: "Redeem" }));
  await expect(page.getByRole("heading", { name: "Pay with your tank" })).toBeVisible();
  await customerClick(page.getByRole("button", { name: "Retailer confirms 20 L" }));
