...
export async function GET() {
  try {
    ...
    return NextResponse.json({ source: "UK Fuel Finder", live: true, persisted, ...snapshot, minimumPrice: stations[0] ?? null, maximumPrice: stations.at(-1) ?? null }, { headers: { "Cache-Control": "public, s-maxage=300, stale-while-revalidate=600" } });
  } catch (error) {
