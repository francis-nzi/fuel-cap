...
export function BusinessOverview({ actorId, role }: { actorId: string; role: string }) {
  ...
  return <div className="business-overview">
    ...
    <div className="business-grid">
      ...
      <section className="business-card"><div className="business-card__heading"><div><span>Coverage stack</span><h2>How price-rise claims are funded</h2></div><strong>1.70×</strong></div><div className="coverage-stack"><div><span>Fees collected</span><strong>{dollars(economics.feesCollectedMinor)}</strong><i aria-hidden="true" style={{width:"70%"}}/></div><div><span>Protection reserve</span><strong>{dollars(acceptedExposureSnapshot.reserveAvailableMinor)}</strong><i aria-hidden="true" style={{width:"100%"}}/></div><div><span>Insurance / paper hedge recovery</span><strong>{dollars(economics.insuranceRecoveryMinor)}</strong><i aria-hidden="true" style={{width:"32%"}}/></div></div><p className="business-note"><ShieldCheck size={15}/> Current accepted exposure remains inside the funded envelope.</p></section>
    </div>
    ...
    <section className="business-card"><div className="business-card__heading"><div><span>Estimated spread risk</span><h2>What happens if fuel prices move?</h2></div><strong>Accepted positions only</strong></div><div className="scenario-table" role="table" aria-label="Price risk scenarios"><div role="row"><b role="columnheader">Scenario</b><b role="columnheader">Expected claims</b><b role="columnheader">Insurance / hedge</b><b role="columnheader">Reserve use</b><b role="columnheader">Position</b></div>{stressProjections.slice(0,5).map((stress)=><div role="row" key={stress.stressId}><span role="cell">{stress.label}</span><strong role="cell">{dollars(stress.expectedClaimsMinor)}</strong><span role="cell">{dollars(stress.simulatedPayoffMinor)}</span><span role="cell">{dollars(stress.reserveUseMinor)}</span><em role="cell" className={`scenario-outcome scenario-outcome--${stress.outcome.toLowerCase()}`}>{stress.outcome}</em></div>)}</div></section>
