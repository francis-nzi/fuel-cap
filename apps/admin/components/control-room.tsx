          ...
          <div className="nav-list">
            {visibleNavigation.map(({ key, label, icon: Icon }) => (
              <button className={`nav-item ${activeWorkspace === key ? "nav-item--active" : ""}`} key={label} type="button" onClick={() => { setActiveWorkspace(key); closeMobileNavigation(); }}>
                <Icon size={17} />
                <span>{label}</span>
                {key !== "control-room" && key !== "customers" && key !== "fleets-vehicles" && key !== "pricing-data" && key !== "spread-fx" && key !== "risk-hedging" && key !== "transactions-ledger" && key !== "billing-reconciliation" && key !== "fraud-cases" && key !== "rules-automation" && key !== "communications" && <span className="nav-soon">Soon</span>}
              </button>
            ))}
          </div>
        </nav>
